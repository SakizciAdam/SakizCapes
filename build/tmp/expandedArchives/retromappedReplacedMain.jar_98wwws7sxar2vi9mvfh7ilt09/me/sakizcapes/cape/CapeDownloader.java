...
public class CapeDownloader {
    ...
    private ResourceLocation getCapeTexture() {
        ...
        switch (SakizCapes.getSettings().getCurrentMode()) {
            case URL:
                location = Minecraft.func_71410_x().func_110434_K()
                        .func_110578_a(Reference.MOD_ID, new DynamicTexture(ImageUtils.getImageFromURL(SakizCapes.getSettings().getCapeURL())));
                break;
            case LOCAL:
                location = Minecraft.func_71410_x().func_110434_K()
                        .func_110578_a(Reference.MOD_ID, new DynamicTexture(ImageUtils.getImageFromFile(SakizCapes.getSettings().getCapePath())));
                break;
            case CLIPBOARD:
                if (!SakizCapes.getSettings().isClipboardSaved()) {
                    BufferedImage clipboard = ImageUtils.getImageFromClipboard();
                    if (clipboard != null) {
                        location = Minecraft.func_71410_x().func_110434_K()
                                .func_110578_a(Reference.MOD_ID, new DynamicTexture(clipboard));
                    }
                } else {
					BufferedImage cape = ImageUtils.getImageFromFile("clipboard.png");
				    location = Minecraft.func_71410_x().func_110434_K()
                                .func_110578_a(Reference.MOD_ID, new DynamicTexture(cape));
				}
                break;
        }
        return location;
